...
@Injectable()
export class MatDayjsDateAdapter extends DateAdapter<dayjs.Dayjs> {
  private _localeData: {
  ...
  constructor(@Optional() @Inject(MAT_DATE_LOCALE) public dateLocale: string,
              @Optional() @Inject(MAT_DAYJS_DATE_ADAPTER_OPTIONS) private options?: MatDayJsDateAdapterOptions) {
    super();
  ...
  override setLocale(locale: string) {
    ...
    const dayJs_LocaleData = this.dayJs().localeData();
    this._localeData = {
      firstDayOfWeek: dayJs_LocaleData.firstDayOfWeek(),
      longMonths: dayJs_LocaleData.months(),
      shortMonths: dayJs_LocaleData.monthsShort(),
      dates: range(31, (i) => this.createDate(2017, 0, i + 1).format('D')),
      longDaysOfWeek: range(7, (i) => this.dayJs().set('day', i).format('dddd')),
      shortDaysOfWeek: dayJs_LocaleData.weekdaysShort(),
      narrowDaysOfWeek: dayJs_LocaleData.weekdaysMin(),
    };
  }
  ...
  isValid(date: string | Dayjs): boolean {
  ...
  private get shouldUseUtc(): boolean {
    const { useUtc }: MatDayJsDateAdapterOptions = this.options || {};
    return !!useUtc;
  }
